...
with tab2:
    ...
    elif st.button("🎯 Analyze My Profile", key="btn_analyze", type="primary", use_container_width=True):
        with st.spinner("🤖 Analyzing your CV..."):
            try:
                from groq import Groq
                client = Groq(api_key=st.secrets["GROQ_API_KEY"])
                prompt = f"You are an expert career coach. Analyze this CV:\n{st.session_state.cv_text[:2000]}"
...
with tab4:
    ...
    if not st.session_state.cv_text:
        ...
        if st.button("✨ Rewrite My CV", key="btn_rewrite", type="primary", use_container_width=True):
            if job_desc:
...
with tab5:
    ...
    if not st.session_state.cv_text:
        ...
        if st.button("✍️ Generate Cover Letter", key="btn_cl", type="primary", use_container_width=True):
